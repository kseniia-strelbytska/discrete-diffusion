...
def create_pad_mask(
    matrix: torch.Tensor, pad_token: int = PAD_token
) -> torch.Tensor:
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, src, timestep=None, mask=None, src_key_padding_mask=None):
        # Src size must be (batch_size, src sequence length)
        # Tgt size must be (batch_size, tgt sequence length)
        
        if mask == None:
            mask = get_tgt_mask(src.size(1), src.device)
        if src_key_padding_mask == None:
            src_key_padding_mask = create_pad_mask(src)
        ...
        # Transformer blocks - Out size = (sequence length, batch_size, vocab_size)
        ...
        return out.permute(1, 0, 2) # (batch_size, sequence length, vocab_size)