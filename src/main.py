...
def setup_experiment_dirs(
    PROJECT_ROOT, MODELS_DIR, FIGURES_DIR, config_path, experiment_path="new_diffusion/"
):
...
def train(
    ...
    for epoch in epochs_iter:
        ...
        if epoch % validation_config.val_every == 0:
            ...
            if verbose:
                print(
                    f"Epoch {epoch+1}/{epochs}, Train Loss: {avg_loss:.4f}, Test Loss: {avg_test_loss:.4f}, Current lr: {current_lr}"
                )
            with open(dirs.loss_log_path, "a") as f:
                f.write(
                    f"Epoch {epoch+1}/{epochs}, Train Loss: {avg_loss:.4f}, Test Loss: {avg_test_loss:.4f}, Current lr: {current_lr}\n"
...
def main():
    ...
    dirs = setup_experiment_dirs(
        PROJECT_ROOT, MODELS_DIR, FIGURES_DIR, args.config, experiment_path_dated
    )
