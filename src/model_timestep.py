...
class TimestepTransformerClassifier(torch.nn.Module):
    ...
    def forward(self, X: torch.Tensor, timestep: torch.Tensor):
        ...
        # Sinusoidal timestep encoding: sigma_map expects (B,), output is (B, E)
        t = timestep.view(B)  # handle (B,1) or (B,) or scalar-per-batch shapes
        c = torch.nn.functional.silu(self.sigma_map(-torch.log(1 - (1 - self.sampling_eps) * t)))
        X += c.unsqueeze(1)  # (B, 1, E) broadcasts over sequence length L
