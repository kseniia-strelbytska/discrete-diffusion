...
# project token scheme (matches constants.py)
SOS_token = np.array([3])
EOS_token = np.array([2])
PAD_token = np.array([4])

# AB-type grammars: aNbN, abN, baN, bbaN, aNbM, aNbNaN
A_token = np.array([0])
B_token = np.array([1])

# three-symbol grammar: aNbNcN
C_token = np.array([6])

# bracket/parenthesis grammars
# parentheses-only: open=0, close=1 (2 content tokens, vocab_size=6)
OPENING_PARENTHESIS_token = np.array([0])
CLOSING_PARENTHESIS_token = np.array([1])
# brackets (and combined grammars): open=6, close=7 (vocab_size=8)
OPENING_BRACKET_token = np.array([6])
CLOSING_BRACKET_token = np.array([7])
    ...
    if n == 0:
        ...
        while len(word) < n:  # Each pair of parentheses or brackets adds 2 characters
            if len(stack) == 0:
                ...
                choice = random.choice([OPENING_BRACKET_token, CLOSING_BRACKET_token])
                if len(word) + len(stack) >= n:
                    choice = CLOSING_BRACKET_token
