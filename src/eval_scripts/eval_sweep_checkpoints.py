...
class TeeStream:
    """
    Wraps a stream (stdout or stderr) and mirrors every write to a log file.
    Flushes the file on every write so the log is always up-to-date on disk.
    """
...
def main():
    ...
    # Open the live log file and tee both stdout and stderr into it.
...
def _run(args, device, out_dir):
    """Main evaluation logic, called after the log tee is installed."""
