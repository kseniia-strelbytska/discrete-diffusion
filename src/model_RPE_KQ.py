...
class RPEMultiheadAttentionLayer(nn.Module):
    ...
    def _sa_block(self, X):
        ...
        R = self.R_embed(torch.arange(start=self.max_len - L, end=self.max_len + L - 1).to(X.device)) # (2L-1, d_head)

        def relative_skew(A):
            """Efficient skew for (B, nhead, L, 2L-1) -> (B, nhead, L, L)"""
            skew = F.pad(A, (0, 1))                          # (B, nhead, L, 2L)
            skew = skew.view(B, self.nhead, L * 2 * L)       # (B, nhead, L * 2L)
            skew = F.pad(skew, (0, L - 1))                   # (B, nhead, 2L^2 + L - 1)
            skew = skew.view(B, self.nhead, L + 1, 2 * L - 1)# (B, nhead, L+1, 2L-1)
            skew = skew[:, :, :L, L - 1:]                    # (B, nhead, L, L)
            return skew

        R_t = R[None, None, :, :].transpose(-2, -1)  # (1, 1, d_head, 2L-1)

        skew_q = relative_skew(Q @ R_t)  # (B, nhead, L, L)
        skew_k = relative_skew(K @ R_t)  # (B, nhead, L, L)

        # skew_k is from the key perspective, so it should be transposed before adding
        attn = (Q @ K.transpose(-2, -1) + skew_q + skew_k.transpose(-2, -1)) / math.sqrt(Q.shape[-1])
...
class RPEKQTransformerClassifier(torch.nn.Module):
    def __init__(
        self,
        max_len=16,
        vocab_size=6,
