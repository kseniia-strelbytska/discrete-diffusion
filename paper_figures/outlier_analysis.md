...
Source: `results/combined_6_grammar.csv` (oracle sweep, 1236 rows). All figures regenerated from this file by `make_paper_figures.py`.
...
- **Degeneracy is collapse, not a T value.** A single repeated valid string can appear at T=1, 2 or 4 (e.g. bbaᴺ greedy at T=2 has uniqueness 0.056). So instead of excluding low-T rows, we compare samplers **at maximum compute**, where greedy is fully iterative and diverse — a degeneracy-free comparison (AR/EB excluded; they reach 1.0 trivially).
  - Under **uniform** (max compute), categorical beats greedy for: ['baN', 'parentheses_and_brackets', 'not_nested_parentheses_and_brackets'] (baᴺ 0.972 vs 0.932).
...
- Uniform categorical at T=1 sits at **0.216** ≈ ½, the chance rate of a single parity bit; uniform greedy at T≤8 is **0.000** (deterministic wrong parity).
...
- bbaᴺ: greedy T=1 accuracy **0.014**, uniqueness **0.1428571428571428** → every sample is the SAME modal string.
...
- High-accuracy categorical points sit at large NFE (median 456, up to 1008 steps), whereas greedy reaches comparable or higher accuracy at a fraction of the compute. This is the core compute–quality asymmetry behind the trade-off figures.

## 4. EB-sampler categorical underperforms on the counting grammars
- aᴺbᴺ EB categorical best = **0.620** vs EB greedy = 1.000. The adaptive entropy-bounded stopping halts while the count is still uncertain under stochastic sampling; greedy is unaffected because it never injects sampling noise. Flag in text; do not present EB as uniformly Pareto-dominant.

## 5. Diversity reliability gate
- **196/1236** rows are flagged `n_correct_too_low` (too few valid samples to estimate diversity); their diversity is set to NaN and omitted from Fig 2/3. Most are low-compute or collapsed-greedy cells. This is why several greedy lines in Fig 2 are short — by construction, not by omission.

## 6. Is more compute always better? (monotonicity check on the envelope)
- 12 envelope line(s) peak at intermediate compute (worth a sentence):
  - baN / gaussian / greedy: acc@max-compute=0.524 vs peak=0.984
  - baN / gaussian / categorical: acc@max-compute=0.726 vs peak=0.996
  - bbaN / uniform / greedy: acc@max-compute=0.996 vs peak=0.998
  - bbaN / gaussian / greedy: acc@max-compute=0.994 vs peak=0.998
  - aNbN / gaussian / categorical: acc@max-compute=0.988 vs peak=0.992
  - aNbNcN / gaussian / greedy: acc@max-compute=0.996 vs peak=1.000
  - aNbNcN / gaussian / categorical: acc@max-compute=0.978 vs peak=0.994
  - parentheses_and_brackets / gaussian / greedy: acc@max-compute=0.878 vs peak=0.998
  - parentheses_and_brackets / gaussian / categorical: acc@max-compute=0.960 vs peak=0.980
  - not_nested_parentheses_and_brackets / uniform / greedy: acc@max-compute=0.974 vs peak=1.000
  - not_nested_parentheses_and_brackets / gaussian / greedy: acc@max-compute=0.898 vs peak=1.000
  - not_nested_parentheses_and_brackets / gaussian / categorical: acc@max-compute=0.952 vs peak=0.990
